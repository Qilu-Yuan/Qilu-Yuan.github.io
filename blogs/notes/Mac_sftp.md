...
## Mac电脑使用CommanderOnePro在Finder上挂载sftp服务器，终端直接打开远程路径

### MacOS 在finer上挂载sftp服务器 

<br>由于本人平时经常需要在远程服务器（sftp）上进行任务提交，数据处理等操作。图形界面应用起来会方便很多，之前大多是在ubuntu上进行操作，可以直接在文件管理器上sftp挂载远程服务器，并且支持在任意位置打开远程路径的terminal界面（文件管理器空白处右键 在远程打开终端）

<br>近期，本人更换了mac电脑，因此摸索了一下如何在macOS上finder直接挂载远程sftp服务器。

<br>初步发现了两款收费软件（commander one pro 和 RemoteDrive）均可以在finder上挂载sftp服务器

<br>以commander one pro为例,我们需要先在commander one pro上连接sftp服务器

<br>右上角连接

<img src="/Figure/Mac_sftp/1.png">

<br>选择ftp

<img src="/Figure/Mac_sftp/2.png">

<br>选择sftp,并填写服务器IP,用户名,密码

<img src="/Figure/Mac_sftp/3.png">

<br>注意:根据后续直接打开远程路径的需要，用户名需填写成username@hostname的形式,username为服务器上的用户名,hostname为服务器ip

<br>点击连接就能成功连接。

#### 在finder内操作

需要在commander one pro的界面上直接双击打开一个文件

然后就能在mac上的用户家目录(本人是/Users/yuanqilu)下的.COVolumes文件夹内找到远程的目录了

### 终端直接打开远程路径

<br>MacOS在Finder上打开显示路径（option+command+P）以后，鼠标右键点击路径就能在终端打开当前文件夹了

<br>为了在终端打开finder上的挂载的路径时能够同时直接进入到远程服务器上路径,本人采用了python脚本来自动进行
...
if (len(Dir)>3) and (Dir[3]=='.COVolumes'):
    host =Dir[4].split('@')[1]

    user =Dir[4].split('@')[0][1:]


    for i in range(5,len(Dir)):
        path = '/'.join(Dir[5:])
    print(path)
    call(f'ssh -t {user}@{host} \"cd /{path};exec bash --login\"', shell=True)
```

运行上述python脚本就能直接进入远程服务器上的当前路径

为了实现完全自动化，我将运行上述脚本的命令写进了环境变量 ~/.zshrc

```bash
python ~/.cdsftp/CdSftp.py
```
